...
public class MyntraScraper implements ScraperStrategy {
    ...
    public Map<String, Object> scrape(String url) {
        ...
        try {
            ...
            // Attempt primary hybrid JSON-LD parsing
            boolean parsedJson = false;
            Elements scripts = doc.getElementsByTag("script");
            for (Element script : scripts) {
                String htmlContent = script.html();
                if (htmlContent.contains("window.pdpData") || htmlContent.contains("pdpData")) {
                    try {
                        ...
                        if (startIndex != -1 && endIndex != -1 && endIndex > startIndex) {
                            ...
                            parsedJson = true;
                            log.info("Myntra parsed successfully via pdpData JSON-LD!");
                            break;
                        }
                    } catch (Exception jsonEx) {
                        log.warn("Failed to parse Myntra pdpData JSON block, falling back to standard selector parsing: {}", jsonEx.getMessage());
                    }
                }
            }
